...
export async function addPost(
  body: API.AddPostCommand,
  options?: { [key: string]: any },
) {
  return request<API.ResponseDTOVoid>('/system/post', {
    method: 'POST',
    headers: {
...
export async function getPostInfo(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getPostInfoParams,
  options?: { [key: string]: any },
) {
  const { postId: param0, ...queryParams } = params;
  return request<API.ResponseDTOPostDTO>(`/system/post/${param0}`, {
    method: 'GET',
    params: { ...queryParams },
    ...(options || {}),
  });
...
export async function editPost(
  ...
  body: API.UpdatePostCommand,
  options?: { [key: string]: any },
) {
  const { postId: param0, ...queryParams } = params;
  return request<API.ResponseDTOVoid>(`/system/post/${param0}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function getPagedPost(
  ...
  return request<API.ResponseDTOPageDTOPostDTO>('/system/post/list', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
