<script setup lang="ts">
    ...
    proxyConfig: {
      ajax: {
        query: async ({page}, formValues = {}) => {
          const {data} = await getPagedEstates({
  ...
  function handleMultiDelete() {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await batchRemoveEstate({estateIds: ids});
        await tableApi.query();
      },
    });
  }
  ...
  function handleDownloadExcel() {
    commonDownloadExcel(exportEstateByExcel, '房屋和构筑物类固定资产数据', tableApi.formApi.form.values, {
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="房屋和构筑物类固定资产列表">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
              ...
              danger
              type="primary"
              v-access:code="['regulatory:estate:remove']"
              @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </a-button>
          <a-button
              type="primary"
              v-access:code="['regulatory:estate:add']"
              @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
              v-access:code="['regulatory:estate:edit']"
