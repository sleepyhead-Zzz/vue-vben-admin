...
const [PlanModal, modalApi] = useVbenModal({
  connectedComponent: planModal,
});
...
async function handleEdit(row: AssetAPI.AssetInspectionPlanDTO) {
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: AssetAPI.AssetInspectionPlanDTO) => row.planId);
  Modal.confirm({
    title: '提示',
    okType: 'danger',
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="巡检计划列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['asset:plan:export']"
    ...
    <PlanModal @reload="tableApi.query()" />
  </Page>
</template>
