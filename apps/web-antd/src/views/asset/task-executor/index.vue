...
async function loadInspectionItem(deviceId: number | string) {
  try {
...
const goToRepair = () => {
  const deviceId = taskInfo.value?.deviceId;
  if (!deviceId) {
    return;
  }
  router.push(`/asset/device/${deviceId}/repair`);
};
</script>
      ...
      <AForm :model="inspectionForm" @finish="onFinish" layout="vertical">
        ...
        <AFormItem class="mt-6">
          <div class="flex justify-end gap-3">
            <AButton v-if="taskInfo?.deviceId" @click="goToRepair">
              提交维修工单
            </AButton>
