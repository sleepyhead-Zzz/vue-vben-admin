...
async function handleEdit(record: SystemAPI.SysOssConfigDTO) {
  drawerApi.setData({ id: record.ossConfigId });
  drawerApi.open();
}

async function handleDelete(row: SystemAPI.SysOssConfigDTO) {
  await removeOssConfig([row.ossConfigId]);
  await tableApi.query();
}

function handleMultiDelete() {
  const rows = tableApi.grid.getCheckboxRecords();
  const ids = rows.map((row: SystemAPI.SysOssConfigDTO) => row.ossConfigId);
  Modal.confirm({
    title: '提示',
    okType: 'danger',
