...
const gridOptions: VxeGridProps = {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        const { data } = await getPagedFactKeyTaskScore({
  ...
  // 表格全局唯一表示 保存列配置需要用到
  id: 'perf-FactKeyTaskScore-index',
};
...
const [FactKeyTaskScoreModal, modalApi] = useVbenModal({
  connectedComponent: keyTaskScoreModal,
});
const [FactKeyTaskScoreImportModal, importModalApi] = useVbenModal({
...
function handleMultiDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await removeFactKeyTaskScore({ FactKeyTaskScoreIds: ids });
      await tableApi.query();
    },
  });
}
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="重点工作得分列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['perf:FactKeyTaskScore:import']"
            @click="handleImport"
          >
            {{ $t('pages.common.import') }}
          </a-button>
          <a-button
            v-access:code="['perf:FactKeyTaskScore:export']"
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['perf:FactKeyTaskScore:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['perf:FactKeyTaskScore:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['perf:FactKeyTaskScore:edit']"
    ...
    <FactKeyTaskScoreModal @reload="tableApi.query()" />
    <FactKeyTaskScoreImportModal @reload="tableApi.query()" />
  </Page>
</template>
