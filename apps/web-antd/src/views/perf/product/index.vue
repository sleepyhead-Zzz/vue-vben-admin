...
import {
  exportProductByExcel,
  getPagedProduct,
  removeProduct,
} from '#/api/perf/product';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        const { data } = await getPagedProduct({
...
function handleMultiDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await removeProduct({ productIds: ids });
      await tableApi.query();
    },
  });
}
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="产品维度列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['perf:product:export']"
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['perf:product:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['perf:product:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['perf:product:edit']"
    ...
    <ProductModal @reload="tableApi.query()" />
  </Page>
</template>
