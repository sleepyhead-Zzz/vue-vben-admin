...
import {
  exportFactManagementScoreByExcel,
...
const gridOptions: VxeGridProps = {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        const { data } = await getPagedFactManagementScore({
  ...
  // 表格全局唯一表示 保存列配置需要用到
  id: 'perf-FactManagementScore-index',
};
...
async function handleEdit(row: PerfAPI.PerfFactManagementScoreDTO) {
  modalApi.setData({ id: row.managementId });
  modalApi.open();
}

async function handleDelete(row: PerfAPI.PerfFactManagementScoreDTO) {
  if (typeof row.managementId !== 'number') {
    return;
  }
  await removeFactManagementScore({
    FactManagementScoreIds: [row.managementId],
  });
...
function handleMultiDelete() {
  ...
  const ids = rows
    .map((row: PerfAPI.PerfFactManagementScoreDTO) => row.managementId)
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await removeFactManagementScore({ FactManagementScoreIds: ids });
      await tableApi.query();
    },
  });
}
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="内部管理得分：周报/样品反馈/执行力等列表">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['perf:FactManagementScore:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['perf:FactManagementScore:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['perf:FactManagementScore:edit']"
