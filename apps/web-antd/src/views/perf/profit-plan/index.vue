  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        const { data } = await getPagedFactProfitPlan({
...
function handleMultiDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await removeFactProfitPlan({ planIds: ids });
      await tableApi.query();
    },
  });
}
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="利润计划列表">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['perf:FactProfitPlan:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['perf:FactProfitPlan:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['perf:FactProfitPlan:edit']"
