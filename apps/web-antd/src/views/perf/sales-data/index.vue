  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        const { data } = await getPagedSalesData({
...
const [SalesDataModal, modalApi] = useVbenModal({
  connectedComponent: salesDataModal,
});
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="销售明细事实列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['perf:factSales:export']"
            @click="handleDownloadExcel"
          >
            {{ $t('pages.common.export') }}
          </a-button>
          <a-button
            v-access:code="['perf:saleData:import']"
            @click="handleImport"
          >
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['perf:factSales:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['perf:factSales:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['perf:factSales:edit']"
    ...
    <SalesDataModal @reload="tableApi.query()" />
    <SaleDataImportModal @reload="tableApi.query()" />
  </Page>
</template>
