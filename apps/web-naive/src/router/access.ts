...
import { getRouters } from '#/api/common/login';
...
async function generateAccess(options: GenerateMenuAndRoutesOptions) {
  ...
  return await generateAccessible(preferences.app.accessMode, {
    ...
    fetchMenuListAsync: async () => {
      // 清除以前的message
      message.destroyAll();
      message.loading(`${$t('common.loadingMenu')}...`, { duration: 1 });
      // 后台返回路由/菜单
      const backMenuList = await getRouters();
      // 转换为vben能用的路由
      const vbenMenuList = backMenuToVbenMenu(backMenuList.data);
      // 特别注意 这里要深拷贝
